...
**Don't use incorrect extends paths in subdirectory controllers:**
```cfm
// ❌ WRONG - Component won't resolve in subdirectories like app/controllers/platform/
component extends="platform.Controller" { }
component extends="Controller" { }

// ✅ CORRECT - Use full component path from app root
component extends="app.controllers.platform.Controller" { }
component extends="app.controllers.Controller" { }
```

**Don't use var for variables that should be available in views:**
```cfm
// ❌ WRONG - Variable is local to function, NOT available in view
function index() {
    var users = model("User").findAll();
}

// ✅ CORRECT - Omit var to make available in view
function index() {
...
**CRITICAL: Normalize boolean checkbox/toggle fields before model operations:**
```cfm
// ❌ WRONG - Checkbox fields with hidden input fallback submit "false,true" when checked
function create() {
    bom = model("BOM").new(params.bom);  // Crashes with "Can't cast String [false,true] to boolean"
}

// ✅ CORRECT - Normalize boolean fields before model creation
function create() {
    // HTML forms with hidden + checkbox submit "false,true" when checked
    ...
    bom = model("BOM").new(params.bom);
}
// NOTE: Add this normalization to BOTH create() and update() actions!
```
...
### CRITICAL: Filter Inheritance in Controller Hierarchies

When using controller inheritance (parent/child controllers), filters defined in parent
controllers apply to ALL child controller actions. Child `except` clauses do NOT override
parent filters - they add additional filters!

```cfm
// ❌ WRONG - Parent filter causes redirect loop on child's login action
// Parent Controller:
component extends="app.controllers.Controller" {
    function config() {
        filters(through="requireAuth");  // Runs for ALL child actions!
    }
}

// Child Controller:
component extends="app.controllers.admin.Controller" {
    function config() {
        super.config();
        // This ADDS another filter, doesn't override parent!
        filters(through="requireAuth", except="new,create");
    }
}

// ✅ CORRECT - Define auth filter ONLY in child controllers
// Parent Controller:
component extends="app.controllers.Controller" {
    function config() {
        // Don't add auth filter here - let children define it
        filters(through="loadCommonData");
    }
}
...
component extends="app.controllers.admin.Controller" {
    function config() {
        super.config();
        // Each child defines its own auth filter with appropriate exceptions
        filters(through="requireAuth", except="new,create");
    }
}
```
