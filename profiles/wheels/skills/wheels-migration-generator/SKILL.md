...
### BCrypt Password Hash Compatibility (CRITICAL)

**🔴 CRITICAL:** When seeding user passwords with BCrypt hashes, always use `$2a$` prefix!

```cfm
// ❌ WRONG - $2b$ and $2y$ prefixes may cause "Invalid salt revision" errors
var passwordHash = "$2b$12$aztqJfZFE0gejfFC8cTR.ubshEX.ND55SvG59En7/gU43iA9TzAKq";
var passwordHash = "$2y$10$abcdef...";

// ✅ CORRECT - $2a$ prefix has universal library compatibility
...
function up() {
    transaction {
        ...
        // BCrypt hash of password - MUST use $2a$ prefix
        // Generated with: BCrypt.hashpw("yourpassword", BCrypt.gensalt(12))
        // Change $2b$ to $2a$ if your generator uses different prefix
        var passwordHash = "$2a$12$aztqJfZFE0gejfFC8cTR.ubshEX.ND55SvG59En7/gU43iA9TzAKq";
...
**Why $2a$?** Different BCrypt implementations use different version prefixes:
- `$2a$` - Original, widely supported
- `$2b$` - OpenBSD version, not all libraries support
- `$2y$` - PHP version, not all libraries support

Lucee/CFML BCrypt libraries typically only recognize `$2a$`.
