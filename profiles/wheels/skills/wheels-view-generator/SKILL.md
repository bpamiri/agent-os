...
## Critical: Controller Variable Scoping

**Variables must be declared WITHOUT `var` in controllers to be available in views!**

```cfm
// ❌ WRONG - Variable is LOCAL to function, NOT available in view
function index() {
    var users = model("User").findAll();  // View can't access this!
}

// ✅ CORRECT - Omit var to expose to view
function index() {
    users = model("User").findAll();  // Available in view as #users#
}
```

**Why:** In Wheels, action methods set view data by assigning to un-scoped variables. Using `var` creates a function-local variable that the view cannot access.
...
### ❌ ANTI-PATTERN 7: Missing hx-select for Partial Updates

**Wheels renders full HTML pages by default. Without `hx-select`, the entire response overwrites the target:**

```html
<!-- ❌ WRONG - Full page response breaks page structure -->
<select hx-get="/reports/data"
        hx-target="#report-content"
        hx-swap="outerHTML">

<!-- ✅ CORRECT - Extract only matching element from response -->
<select hx-get="/reports/data"
        hx-target="#report-content"
...
### JavaScript htmx.ajax() Pattern

```javascript
// ❌ WRONG - Missing select option
htmx.ajax('GET', baseUrl + '?' + params.toString(), {
    target: '#report-content',
    swap: 'outerHTML'
});

// ✅ CORRECT - Include select option
htmx.ajax('GET', baseUrl + '?' + params.toString(), {
    target: '#report-content',
...
### HTMX-Enabled View Structure

```cfm
<cfoutput>
<!--- Wrapper div with ID that matches hx-target and hx-select --->
<div class="p-4 lg:p-6" id="report-content">
    <!--- Report header --->
    ...
    <div class="screen-only">
        <select id="filterSelect"
                hx-get="#urlFor(route='myReport')#"
                hx-target="##report-content"
                hx-select="##report-content"
                hx-swap="outerHTML"
                hx-push-url="true"
                hx-include="[name='otherParam']"
...
**Key Points:**
- Always use `hx-select` when targeting a partial area of the page
- The `hx-select` value should match the `hx-target` selector
- Use `hx-push-url="true"` to update browser URL for bookmarkable states
- Use `hx-include` to include additional form fields in the request
...
- [ ] **HTMX**: Include `hx-select` alongside `hx-target` for partial updates
- [ ] **HTMX**: Use `select` option in `htmx.ajax()` JavaScript calls
